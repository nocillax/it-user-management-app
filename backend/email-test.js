...
async function sendTestEmail(email) {
  try {
    if (!process.env.JWT_SECRET) {
      console.error(
        "❌ JWT_SECRET is not set. Cannot generate verification token."
      );
      return false;
    }
    ...
    console.log(
      `Verification URL would be: ${process.env.FRONTEND_URL}/verify/${verificationToken}`
    );
    return true;
  } catch (error) {
...
async function runTests() {
  ...
  if (configValid) {
    ...
    readline.question(
      "\nEnter an email address to send a test verification email: ",
      async (email) => {
